...
def create_task(task: TaskCreate):
    ...
    # Verify board exists
    cur.execute("SELECT id FROM boards WHERE id = %s", (task.board_id,))
    if not cur.fetchone():
        cur.close()
        conn.close()
...
def get_tasks(board_id: Optional[int] = Query(None)):
    conn = get_connection()
    cur = conn.cursor()
...
def update_task(task_id: int, task: TaskUpdate):
    ...
    cur.execute("SELECT id, board_id, title, description, status, created_at FROM tasks WHERE id = %s", (task_id,))
    row = cur.fetchone()
    if not row:
        cur.close()
        conn.close()
